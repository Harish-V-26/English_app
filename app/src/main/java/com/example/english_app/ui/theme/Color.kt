package com.example.english_app.ui.theme

import androidx.compose.ui.graphics.Color

// Vibrant English Learning App Colors
val VibrantBlue = Color(0xFF2196F3)
val VibrantGreen = Color(0xFF4CAF50)
val VibrantOrange = Color(0xFFFF9800)
val VibrantPurple = Color(0xFF9C27B0)
val VibrantPink = Color(0xFFE91E63)
val VibrantYellow = Color(0xFFFFEB3B)
val VibrantRed = Color(0xFFF44336)
val VibrantTeal = Color(0xFF009688)

// Gradient colors
val GradientStart = Color(0xFF667eea)
val GradientEnd = Color(0xFF764ba2)
val GradientOrangeStart = Color(0xFFf093fb)
val GradientOrangeEnd = Color(0xFFf5576c)
val GradientGreenStart = Color(0xFF4facfe)
val GradientGreenEnd = Color(0xFF00f2fe)

// Background colors
val LightBackground = Color(0xFFF8F9FF)
val CardBackground = Color(0xFFFFFFFF)
val SurfaceVariant = Color(0xFFF0F2FF)

// Text colors
val PrimaryText = Color(0xFF1A1A2E)
val SecondaryText = Color(0xFF6B7280)
val AccentText = Color(0xFF3B82F6)

// Legacy colors for compatibility
val Purple80 = Color(0xFFD0BCFF)
val PurpleGrey80 = Color(0xFFCCC2DC)
val Pink80 = Color(0xFFEFB8C8)

val Purple40 = Color(0xFF6650a4)
val PurpleGrey40 = Color(0xFF625b71)
val Pink40 = Color(0xFF7D5260)
